...
log_file1 = '/home/Model/logs/model_server_logs/new_model_logs'
log_file2 = '/home/Model/logs/model_server_logs/new_rule_logs'
        ...
        if text['data']:
                    ...
                    for i in range(len(text["data"]["scorpionOriginalData"]["calls"])):
                        for j in range(len(text["data"]["scorpionOriginalData"]["calls"][i]["items"])):
                            ...
                            call_info["dial_type"] = text["data"]["scorpionOriginalData"]["calls"][i]["items"][j][
                                "dial_type"]
                            call_info["location_type"] = text["data"]["scorpionOriginalData"]["calls"][i]["items"][j][
                                "location_type"]
            ...
            if call_list:
                ...
                try:
                    ...
                    logger1.info(traceback.format_exc())
                logger1.info('{0},静默超过1天次数,silent_ge_1:{1},静默超过2天次数,silent_ge_2:{2}'.format(zw_order, str(silent_ge_1),
                                                                                            str(silent_ge_2)))
                ...
                try:
                    ...
                    contimes_called_30_90_rate_new = float(month1_called_update) / float(month3_called_update) if float(
                    ...
                    logger1.info('{0},最近3个月被叫个数,month3_called:{1},6个月的被叫个数,month6_called:{2},比值:{3}'.format(zw_order,
                                                                                                            str(float(
                                                                                                                month3_called)),
                                                                                                            str(float(
                                                                                                                month6_called)),
                    ...
                    logger1.info('{0},最近1个月被叫个数,month1_called:{1},3个月的被叫个数,month3_called:{2},比值:{3}'.format(zw_order,
                                                                                                            str(float(
                                                                                                                month1_called)),
                                                                                                            str(float(
                                                                                                                month3_called)),
                ...
                model = {
                    ...
                    "orderfee_180d": float((orderfee_180d[0][0])) if orderfee_180d[0][0] != None else float(99999999),
...
def rule_data(modeldata):  ##假定传入的是json
    ...
    antifraud_message = {"Y1001": "accept", "Y2001": "accept", "Y2002": "accept", "Y2003": "accept",
                         ...
                         "Y3016": "accept",
                         "Y0317": "accept", "Y3018": "accept", "Y3019": "accept", "Y3020": "accept", "Y3021": "accept",
                         'Y3023': "accept",
        ...
        if modeldata['data']['isAgain'] == 0:  ##初借客户
            ...
            if modeldata['data']['scorpionAccessReport']:  ###是否有脱敏,----mobile 具体是什么
                ...
                if modeldata['data']['scorpionOriginalData']['name'].find('*') < 0 and len(
                        ...
                        modeldata['data']['name']:
                    limit_message['S1002'] = 'reject'
            elif modeldata['data']['loanBondData']:  ###是否有脱敏----('状态有效且已经实名制且入网时长超过半年)
            ...
            try:
                if modeldata['data']['loanBondSuolunData']:
                    if int(modeldata['data']['loanBondSuolunData']['risk_social_network'][
                               'sn_order1_blacklist_contacts_cnt']) >= 3:
                        limit_message['H1009'] = 'reject'
                    if int(modeldata['data']['loanBondSuolunData']['user_basic']['used_idcards_cnt']) >= 3:
                        limit_message['H1010'] = 'reject'
                    if int(modeldata['data']['loanBondSuolunData']['user_basic']['used_phones_cnt']) >= 5:
                        limit_message['H1011'] = 'reject'
                    if int(modeldata['data']['loanBondSuolunData']['history_search'][
            ...
            try:
                jjlxr_json_phone = [phone for phone in
                                    [modeldata['data']['contacts'][0].get('phone1'),
                                     modeldata['data']['contacts'][1].get('phone2')]
                                    if phone]
            except:
                jjlxr_json_phone = []
            ...
            if modeldata['data']['scorpionOriginalData']:
                ...
                ##入网时长  数据缺失是null还是‘’？
                try:
                    if modeldata['data']['loanBondOrignalData']['data'][0]['basic']['reg_time']:
                        if modeldata['data']['loanBondOrignalData']['data'][0]['basic']['reg_time'].replace(" ",
                                                                                                            "") == "" or \
                                int(modeldata['data']['order_time']) - time.mktime(
                            time.strptime(modeldata['data']['loanBondOrignalData']['data'][0]['basic']['reg_time'],
                                          '%Y-%m-%d %H:%M:%S')) < 180 * 24 * 60 * 60 or \
                                modeldata['data']['loanBondOrignalData']['data'][0]['basic']['reg_time'].replace(' ',
                                                                                                                 '') == '':
                            antifraud_message['Y1001'] = 'reject'
                except:
                ...
                ##短信相关
                try:
                    if modeldata['data']['loanBondOrignalData']['data'][0]['smses']:
                        if len(modeldata['data']['loanBondOrignalData']['data'][0]['smses']) == 0:  ##无短信内容
                            antifraud_message['Y2001'] = 'reject'
                        if len(modeldata['data']['loanBondOrignalData']['data'][0]['smses']) != 0 and len(  ##短信记录过少
                                modeldata['data']['loanBondOrignalData']['data'][0]['smses']) < 30:
                            antifraud_message['Y2003'] = 'reject'
                except:
                ...
                ##通话记录相关
                try:
                    loanBondOrignalData_call_list = modeldata['data']['loanBondOrignalData']['data'][0]['calls']
                    if len(loanBondOrignalData_call_list) == 0:  # 通话记录缺失（实际贷后邦数据不会存在此情形）
                        ...
                        dail90 = [loanBondOrignalData_call_list[i]['init_type'] for i in
                                  ...
                                  int(modeldata['data']['order_time']) - time.mktime(
                                      time.strptime(loanBondOrignalData_call_list[i]['start_time'],
                                                    '%Y-%m-%d %H:%M:%S')) <= 90 * 24 * 60 * 60]
                        if len([i for i in dail90 if i.find(u'主叫') >= 0]) <= 3:  # 近90天月均主叫次数不超过1次
                            antifraud_message['Y3012'] = 'reject'
                        # 最近一次通话与订单时间间隔超过15天
                ...
                ####以下是贷后邦特有标签
                try:
                    behavior_check = modeldata['data']['loanBondData']['data']['behavior_check']
                    if int(re.sub("\D", "",
            ...
            try:
                if modeldata['data']['mailList']:
                    ...
                    txl_json_phone = [i for i in list(
                        set(list(map(lambda x: get_value(x, 'phone'), modeldata['data']['mailList']))))
                                      if
                                      i.replace('+86', "").replace('+', '').replace(' ', '').replace('-',
                                                                                                     '')]  ##手机通讯录对应的手机号码
                    ...
                    if len(scorpionOriginalData_call_list2) > 0:
                        ...
                        peer_number_last90days = [
                            ...
                            phone in [loanBondOrignalData_call_list[i].get('other_cell_phone') for i in
                                      list(range(len(loanBondOrignalData_call_list))) if \
                                      int(modeldata['data']['order_time']) - time.mktime(
                                          time.strptime(loanBondOrignalData_call_list['start_time'],
                                                        '%Y-%m-%d %H:%M:%S')) < 90 * 24 * 60 * 60]]
                        peer_number_last90daystop20 = list(
                            map(lambda x: x[0], sorted(dict(Counter(peer_number_last90days)).items(),
                                                       key=lambda x: x[1], reverse=True)))[:20]

                if len(list(set(list(map(lambda x: x.get('phone'), modeldata['data']['mailList']))))) < 30:  # 通讯录手机号个数
                    antifraud_message['T1004'] = 'reject'
                if len(list(map(lambda x: len(x), txl_json_phone))) / len(txl_json_phone) <= 0.5:  ###11位手机号占比过少
                    antifraud_message['T1005'] = 'reject'
                if len([phone for phone in
                        list(map(lambda x: x.replace('+86', "").replace('+', '').replace(' ', '').replace('-', ''),
                                 jjlxr_json_phone)) if phone in txl_json_phone]) == 0:  ###至少一个紧急联系人号码不在通讯录中!!!!
                    antifraud_message['T1003'] = 'reject'
                if len([warn for warn in
                        [max(list(
                            map(lambda x: x[i], [list(map(lambda x: x.find(i), txl_json_name)) for i in warn_words])))
                            for
                ...
                if len([relation for relation in
                        [max(list(
                            ...
                            for i in list(range(len(txl_json_name)))] if relation >= 0]) < 1:  ##通讯录不带有亲属类称谓联系人
                ...
                if len([phone for phone in peer_number_last90daystop20 if
                    ...
                    if len([phone for phone in txl_json_phone if phone in peer_number_last90daystop20]) < 3:  ##通讯录真实性
            ...
            try:
                if modeldata['data']['contacts_his']:
                    if len([modeldata['data']['contacts_his'][i] for i in
                            list(range(len(modeldata['data']['contacts_his']))) if \
                            int(modeldata['data']['order_time']) - time.mktime(
                                time.strptime(modeldata['data']['contacts_his'][i]['start_time'],
                                              '%Y-%m-%d %H:%M:%S')) < 3 * 60 * 60]) >= 1:  ##紧急联系人近3小时关联联系人过多(确认当前数据未落入是生产库)
                        antifraud_message['J1001'] = 'reject'
                    if len([modeldata['data']['contacts_his'][i] for i in
                            list(range(len(modeldata['data']['contacts_his']))) if \
                            int(modeldata['data']['order_time']) - time.mktime(
                                time.strptime(modeldata['data']['contacts_his'][i]['start_time'],
                                              '%Y-%m-%d %H:%M:%S')) < 72 * 60 * 60]) >= 2:  ##紧急联系人近72小时关联联系人过多(确认当前数据未落入是生产库)
                        antifraud_message['J1002'] = 'reject'
                    if len([modeldata['data']['contacts_his'][i] for i in
                            list(range(len(modeldata['data']['contacts_his']))) if \
                            int(modeldata['data']['order_time']) - time.mktime(
                                time.strptime(modeldata['data']['contacts_his'][i]['start_time'],
                                              '%Y-%m-%d %H:%M:%S')) < 30 * 24 * 60 * 60]) >= 3:  ##紧急联系人近1个月关联联系人过多(确认当前数据未落入是生产库)
                        antifraud_message['J1003'] = 'reject'
            except:
                logger.INFO('{0},紧急联系人关联他人紧急联系人订单解析错误'.format(modeldata['orderNo']))
            try:
                if modeldata['data']['contacts_order']:
                    if len([modeldata['data']['contacts_order'][i] for i in
                            list(range(len(modeldata['data']['contacts_order']))) if \
                            int(modeldata['data']['order_time']) - time.mktime(
                                time.strptime(modeldata['data']['contacts_order'][i]['start_time'],
                                              '%Y-%m-%d %H:%M:%S')) < 3 * 24 * 60 * 60]) >= 1:  ##紧急联系人近3小时关联联系人过多(确认当前数据未落入是生产库)
            ...
            try:
                if modeldata['data']['deviceId_his']:
                    if len([modeldata['data']['deviceId_his'][i] for i in
                            ...
                            int(modeldata['data']['order_time']) - time.mktime(
                                time.strptime(modeldata['data']['deviceId_his'][i]['start_time'],
                                              '%Y-%m-%d %H:%M:%S')) < 3 * 60 * 60]) >= 1:  ##紧急联系人近3小时关联联系人过多(确认当前数据未落入是生产库)
                    ...
                    if len([modeldata['data']['deviceId_his'][i] for i in
                            ...
                            int(modeldata['data']['order_time']) - time.mktime(
                                time.strptime(modeldata['data']['deviceId_his'][i]['start_time'],
                                              '%Y-%m-%d %H:%M:%S')) < 72 * 60 * 60]) >= 3:  ##紧急联系人近72小时关联联系人过多(确认当前数据未落入是生产库)
                ...
                if modeldata['data']['app_list']:
                    if modeldata['data']['app_list'] != 'null':
                        if max(list(reduce(lambda x, y: x + y,
                                           [list(map(lambda t: t.find(i), json.loads(modeldata['data']['app_list'])))
                                            for i
                                            in app_words]))) >= 0:
            ...
            return {'orderNo': modeldata['data']['orderNo'], 'limit_message': limit_message,
