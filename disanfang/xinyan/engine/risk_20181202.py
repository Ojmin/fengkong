...
'''
20181204 李伟梁 近7天/14天查询次数 改为查询机构数（贷后邦索伦数据）；通讯录号码下降至20；芝麻分不对-1与0分处理,同时<=550改为小于550
20181204  李伟梁 删除两个紧急联系人均不在通讯录规则，近7天查询机构机构数10，近14天查询机构数20
20181204 李伟梁 删除近14天查询机构数，近7天机构数阈值变为8天
20181204 李伟梁 去重通讯录与近3个月top20通话号码交集小于3个改为小于2个
'''
        ...
        if modeldata['data']['isAgain'] == 0:  ##初借客户
            ...
            try:
                if modeldata['data']['loanBondSuolunData']:
                    ...
                    if modeldata['data']['loanBondSuolunData']['history_search'][
                        'org_cnt_recent_7_days'] != '' and int(
                        modeldata['data']['loanBondSuolunData']['history_search'][
                            'org_cnt_recent_7_days']) >= 8:
                        limit_message['H1012'] = 'reject'
                    if modeldata['data']['loanBondSuolunData']['history_search'][
                        'org_cnt_recent_14_days'] != '' and int(
                        modeldata['data']['loanBondSuolunData']['history_search'][
                            'org_cnt_recent_14_days']) >= 20:
                        limit_message['H1013'] = 'accept'
            ...
            try:
                if modeldata['data']['score']['zhima']:
                    if int(modeldata['data']['score']['zhima']) <550 and int(modeldata['data']['score']['zhima']) !=-1\
                            and int(modeldata['data']['score']['zhima']) !=0:  ###芝麻分过低  ()
                        limit_message['H1006'] = 'reject'
            except:
                    ...
                    if not txl_json_name or not txl_json_phone:
                        ...
                        if not modeldata['data']['scorpionOriginalData'] and not modeldata['data'][
                            ...
                            if len(list(set(list(
                                    map(lambda x: x.get('phone'), modeldata['data']['mailList']))))) < 20:  # 通讯录手机号个数
                            ...
                            if len([phone for phone in
                                ...
                                antifraud_message['T1003'] = 'accept'
            ...
            return {'orderNo': modeldata['data']['orderNo'], 'limit_message': limit_message,'isAgain':0,
                    ...
                    'idCardNo': modeldata['data']['idCardNo'],
                    'channel': modeldata['data']['channel'],
